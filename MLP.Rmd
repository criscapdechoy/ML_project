...
```{r}
model.nnet0 <- multinom(nutriscore_grade ~., data = mydataset, subset=learn, maxit=200)
```
...
```{r}
model.nnet <- nnet(nutriscore_grade ~., data = mydataset, subset=learn, size=2, maxit=200)
```
...
- i1 till i6 are the 3 inputs, h1, h2 are the two hidden neurons, b is the bias (offset)
...
```{r}
model.nnet <- nnet(nutriscore_grade ~., data = mydataset, subset=learn, size=2, maxit=200, decay=0.5)
```
...
Now optimize regularization parameter `decay` with  set to fixed 16 units (be patient.. this takes a while..):
```{r}
model.10x10CV <- train (nutriscore_grade ~., data = mydataset, subset=learn, method='nnet', 
                        maxit = 500, trace = FALSE,
                        tuneGrid = expand.grid(.size=16,.decay=decays), trControl=trc)
```
...
in this case it corresponds to 16 hidden neurons, with a decay of `r model.10x10CV$bestTune$decay`
